...
var builder = {
  ...
  rollup: function (callback) {
    ...
    async.each(rollups, function (rollup) {
      ...
      if (!config.combo) {
        content = content.replace(/'skinnable' : true,/g, '');
      }
   ...
   * @param properties {obejct} The properties for the module.
   * @param skin {string} The current skin, it can be undefined.
   * @param callback {function} The callback to be called with the results.
   */
  buildCss: function (data, targetPath, filename, moduleName, properties, skin, callback) {
    // Replace all the ```replace--XXX``` properties.
    ...
    content = templates.css({
      body: content,
      name: skin? 'skin-' + skin + '-' + moduleName : moduleName
    });
